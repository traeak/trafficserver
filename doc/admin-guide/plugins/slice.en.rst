...
The slice plugin supports the following options::
    ...
    --effective-url (optional)
        Use effective url instead of pristine url for remapping.
...
To get around this the '--effective-url' or '-e' option may be used.
This expects an intermediate loopback remap to be defined.

This works well with any remap rules that use the url_sig or
uri_signing plugins.  As the client remap rule is not caching any
plugins that manipulate the cache key would need to go into the
loopback to parrent remap rule.

Examples::

    map http://ats/ http://loopback/ @plugin=slice.so @pparam=--effective-url
    map https://ats/ http://loopback/ @plugin=slice.so @pparam=--effective-url
    map http://loopback/ http://parent/ @plugin=cache_range_requests.so

Or alternatively::

    map http://ats/ http://loopback/ @plugin=slice.so @pparam=-e
    map https://ats/ http://loopback/ @plugin=slice.so @pparam=-e
    map http://loopback/ http://parent/ @plugin=cache_range_requests.so
