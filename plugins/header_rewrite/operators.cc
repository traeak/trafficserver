...
OperatorSetNextHopStrategy::exec(const Resources &res) const
{
  ...
  if ("null" == value || value.empty()) {
    ...
    TSHttpTxnNextHopStrategySet(res.txnp, nullptr);
    return;
  }

  void const *const stratptr = TSHttpTxnNextHopNamedStrategyGet(res.txnp, value.c_str());
  if (nullptr == stratptr) {
    TSWarning("[%s] Failed to get strategy '%s'", PLUGIN_NAME, value.c_str());
  } else {
    Dbg(pi_dbg_ctl, "   Setting strategy '%s'", value.c_str());
    TSHttpTxnNextHopStrategySet(res.txnp, stratptr);
  }
}
