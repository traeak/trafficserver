...
handleFirstServerHeader(Data *const data, TSCont const contp)
{
  ...
  if (TS_HTTP_STATUS_PARTIAL_CONTENT != header.status()) {
    ...
    DEBUG_LOG("Passthru bytes: header: %d body: %" PRId64, hlen, clen);

    if (clen == INT64_MAX) {
      TSVIONBytesSet(output_vio, clen);
    } else {
      int64_t const plen = int64_t(hlen) + clen;
      if (plen <= 0) {
        ERROR_LOG("passthru plen <= 0\n");
        return HeaderState::Fail;
      }
      TSVIONBytesSet(output_vio, plen);
    }
  ...
  if (send416) {
    ...
    DEBUG_LOG("constructed 416 bytes: header: %d body: %" PRId64, hlen, blen);

    int64_t const plen = int64_t(hlen) + blen;
    if (plen <= 0) {
      ERROR_LOG("constructed 416 plen <= 0");
    } else {
      TSVIONBytesSet(output_vio, plen);
      TSHttpHdrPrint(header.m_buffer, header.m_lochdr, output_buf);
  ...
  if (TS_HTTP_STATUS_PARTIAL_CONTENT == data->m_statustype) {
    ...
    if (!crstat) {
      ...
      ERROR_LOG("Bad/invalid response content range '%.*s'", rangelen, rangestr);
      ERROR_LOG("ContentRange %" PRId64 " %" PRId64, data->m_req_range.m_beg, data->m_req_range.m_end);
      ERROR_LOG("First header\n%s", header.toString().c_str());
  ...
  int const hlen = TSHttpHdrLengthGet(header.m_buffer, header.m_lochdr);
  DEBUG_LOG("repsonse bytes: header: %d body: %" PRId64, hlen, bodybytes);

  int64_t const plen = int64_t(hlen) + bodybytes;

  if (plen <= 0) {
    ERROR_LOG("(hbytes + bodybytes) <= 0");
    state = HeaderState::Fail;
  } else {
    TSVIONBytesSet(output_vio, plen);
    data->m_bytestosend = plen;
    TSHttpHdrPrint(header.m_buffer, header.m_lochdr, output_buf);
