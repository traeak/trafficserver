...
handle_client_req(TSCont contp, TSEvent event, Data *const data)
{
  ...
  default: {
    //    fprintf(stderr, "%p handle_client_req unhandled event %s\n", data, TSHttpEventNameLookup(event));
...
{
  // fprintf(stderr, "%p handle_client_resp %s\n", data, TSHttpEventNameLookup(event));
    ...
    if (Data::BlockState::Pending == data->m_blockstate) {
      ...
      if (data->m_config->m_throttle) {
        ...
        if (threshout < (output_done - output_sent)) {
          ...
          fprintf(stderr, "%p handle_client_resp: throttling %zu\n", data, (output_done - output_sent));
        }
      }
  ...
  case TS_EVENT_VCONN_WRITE_COMPLETE: {
    ...
    if (!data->m_upstream.m_read.isOpen()) {
      //      fprintf(stderr, "%p handle_client_resp: shutting down\n", data);
      shutdown(contp, data);
    }
  } break;
  default: {
