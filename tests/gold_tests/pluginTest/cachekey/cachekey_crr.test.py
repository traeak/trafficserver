...
ts.Disk.remap_config.AddLines([
    ...
    ' @plugin=cachekey.so @pparam=--canonical-prefix @pparam=--static-prefix=http://crr @pparam=--crr-compat-range @pparam=--percent-encode=false @plugin=cache_range_requests.so @pparam=--no-modify-cachekey'
])
...
cachekey_exp = 'http://crr/path~foo-bytes=0-'.format(server.Variables.Port)
...
ps.ReturnCode = 0
ps.Streams.stdout.Content = Testers.ContainsExpression("X-Cache-Key: " + cachekey_exp, "expected cachekey " + cachekey_exp)
