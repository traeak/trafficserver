'''Verify traffic_ctl config reload picks up a changed rule file of a remap instance.'''
...
ts.Disk.remap_config.AddLine(
    f'map http://example.com/ http://127.0.0.1:{server.Variables.Port}/ @plugin=modsecurity.so @pparam={RULES_FILE}')
...
reload = Test.AddConfigReload(
    ts, expect_tasks={'remap': 'success'}, delay_start=1, description='Replace the rule file and reload the configuration')
reload.Disk.File(
